...
case "$(echo "$CI_TAG" | sed -n 's/^.*-bump-\([a-z]\+\)-feature-gates-.*$/\1/p')" in
    "patch")
...
featureGates="$(echo "$CI_TAG" | sed -n 's/^.*-feature-gates-\([a-zA-Z0-9+_-]\+\)-date.*$/\1/p')"
if [[ -z "$featureGates" ]]; then
    echo "Error: Could not extract feature gates from CI_TAG: $CI_TAG"
    exit 1
fi
featureGatesLine="$(generate_feature_flags_line "$featureGates")" || exit 1
