    ...
    func prepare(webView: WKWebView, for itemIds: Set<String>, libraryId: LibraryIdentifier, styleId: String, localeId: String) -> Single<()> {
        ...
        return loadStyleData(for: styleId)
            ...
            .flatMap({ schema -> Single<(String, String)> in
                return self.loadItemJsons(for: itemIds, libraryId: libraryId).flatMap({ .just((schema, $0)) })
            })
            .flatMap({ schema, itemJsons -> Single<String> in
                return citationWebViewHandler.getItemsCsl(from: itemJsons, schema: schema)
            })
